...
@section('search')
    ...
    <form class="sidebar-form">
        ...
        <div class="input-group">
            <input type="text" name="q" id="searchh" class="form-control" placeholder="Search by Title...">
                    ...
                    <div class="box-body ">
                        <table id="example1" class="table table-bordered table-striped">
                            ...
                            @foreach($rows as $row)
                                @php
                                    $post_id = $row->id;
                                        $likes = \App\Models\CopticLike::where('post_id' ,$post_id  )
                                        ->where('like', 1)->count();
                                    $dislikes =\App\Models\CopticLike::where('post_id' ,$post_id  )
                                        ->where('like' , 0)->count();
                                @endphp
                                <tr>
                                    <td>{{$row->id}}</td>
                                    <td>{{$row->title}}</td>
                                    <td><a href="{{route('singlepost', ['id' =>$row->id])}}">
                                            <button type="submit" class="btn btn-primary">Show</button>
                                        </a></td>

                                    <td><a href="{{route('coptic.comments', ['id' =>$row->id])}}">
