...
@section('content')
    ...
    <div class="box-body">
        ...
        <form action="{{route(''.$routeName.'.update' , ['id' => $row])}}" method="post" enctype="multipart/form-data">
            ...
            <!-- /. tools -->
            <div class="form-group">
                <textarea id="editor1 " class="form-control" name="post" rows="10" cols="250"
                          placeholder="Write New Post Here">{{$row->post}}</textarea>
            </div>
            <div>
            <img src="{{url('uploads/'.$row->img)}}" name="OldImage" width="400px" height="200px" ><br>
            </div>
